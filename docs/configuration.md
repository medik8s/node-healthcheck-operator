...
| _minHealthy_             | no                                    | 51%                                                                                             | The minimum number of healthy nodes selected by this CR for allowing further remediation. Percentage or integer number. Negative values (both integers and percentages) can be used to configure N-1, N-2, or similar thresholds when the total number of selected nodes is not known in advance. |
